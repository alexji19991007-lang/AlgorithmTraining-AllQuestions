...
// LeetCode 301
public class F010_RemoveInvalidParentheses {
    // TC: O(2^n), in the worst case, we have only left parentheses (n), then for each
    //     of them, either remove or not.
    // SC: O(n), we have to go a maximum depth of n before hitting the base case. Note
    //     that we are not considering the space required to store the valid expressions.
